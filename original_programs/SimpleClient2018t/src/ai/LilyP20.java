 ...
 *
 * 春秋の動作が次の通り<br>
 * 全力で殴り合う場合の得失点差を見る<br>
 * 勝てるならそのとおり動く<br>
 * そうでなければいつもどおり<br>
 *
 * @author niwatakumi
 */
public class LilyP20 extends TajimaLabAI {
    ...
    private static final int POWER_MODE = 5;    // 全力で殴り合う
     ...
     * @param game
     */
    public LilyP20(Game game) {
        super(game);
        this.myName = "Lily-P20";
    ...
    private void modeChange(int mode) {
        ...
        switch (mode) {
            ...
            case AWARD_MODE:
                ...
                break;
            case POWER_MODE:
                this.addMessage("mode : POWER_MODE");
                this.scoreValue = 1.0;
                this.startPlayerValue = 0.0;
                this.resourceValue = 1.0;       // 全力で殴る
                break;
            case FINAL_MODE_1:
    ...
    protected Double evaluateBoard(Game game, int playerNum, Action action) {
        ...
        switch (this.mode) {
            ...
            case POWER_MODE: {
                boolean isStartPlayer = cloneGame.getStartPlayer() == this.myNumber;
                evaluation += calcEvaluateForPower(resources, isStartPlayer);
                break;
            }
            default:
    ...
    private Double calcEvaluateForPower(GameResources[] resources, boolean isStartPlayer) {
        // リソースに応じて評価値を計算
        Double evaluation = 0.0;
     ...
     * 全力で殴り合う
     *
     * @return 全力で殴り合って勝てるならそのアクション，負けるならnull
     */
    private Action powerThink() {
        // パワーモードに変更
        int beforeMode = this.mode;
        if (beforeMode != FINAL_MODE_1 && beforeMode != FINAL_MODE_2) {
            this.modeChange(POWER_MODE);
        } else {
            return null;
        }

        this.addMessage("~~~~~~~~~~~~~~~~~~~~~~~~~~~");
        this.addMessage("~~~~~~~~ POWER think ~~~~~~~~");
        this.addMessage("~~~~~~~~~~~~~~~~~~~~~~~~~~~");
     ...
     * 表彰が取れるならその手をチェック
     *
     * @return bestAction，表彰が取れないならnull
     */
    private Action awardThink() {
        Action bestAction = null;
     ...
     * @return bestAction
     */
    private Action normalThink() {

        this.addMessage("===========================");
        this.addMessage("======== Normal think ========");
        this.addMessage("===========================");
    ...
    protected void think() {
        ...
        // 表彰用パスを取得
        bestAction = this.awardThink();
        // 春秋or表彰が取れないor表彰取れるが自由探索なら全力検索
        if (bestAction == null) {
            bestAction = powerThink();
        }
        // それでもnull=殴り合いに勝てない
        if (bestAction == null) {
            bestAction = normalThink();
        }
